...
while serving:
    ...
    if amount <= 0:
        print("Invalid amount. Plese enter a positive number.\n")
        continue
    ...
    while idx <= 6:
        ...
        if count > 0:
            print(f" {count} x {value}-units note(s) = {count * value}")
...
print(f"Total dispensed : {total_dispendsed}units")
